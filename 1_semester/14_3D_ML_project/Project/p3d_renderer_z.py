...
class Pytorch3dRenderer(object):
    ...
    def render(self, verts_body, faces_body, verts_garment, faces_garment, bg_img, texture_rgb):
        ...
        # get garment and body meshes
        body_mesh = self.get_mesh(verts_body, faces_body, bg_img)
        garment_mesh = self.get_mesh(verts_garment, faces_garment, bg_img, texture_rgb)
        
        # render garment only in front of body
        fragments_body = renderer.rasterizer(body_mesh)
        body_z = fragments_body.zbuf.squeeze(3).squeeze(0)    
        
        fragments_garment = renderer.rasterizer(garment_mesh)
        garment_z = fragments_garment.zbuf.squeeze(3).squeeze(0)
        garment_ptf = fragments_garment.pix_to_face.squeeze(3).squeeze(0)
        garment_bc = fragments_garment.bary_coords.squeeze(3).squeeze(0)
        garment_dists = fragments_garment.dists.squeeze(3).squeeze(0)
        
        mask = garment_z > body_z
        mask3 = torch.movedim(mask.repeat(3, 1, 1), 0, 2)

        garment_z_buf = garment_z * mask
        garment_z_buf[garment_z_buf == 0] = -1
        garment_z_buf = garment_z_buf.unsqueeze(2).unsqueeze(0)

        garment_ptf_buf = garment_ptf * mask
        garment_ptf_buf[garment_ptf_buf == 0] = -1
        garment_ptf_buf = garment_ptf_buf.unsqueeze(2).unsqueeze(0)

        garment_dists_buf = garment_dists * mask
        garment_dists_buf[garment_dists_buf == 0] = -1
        garment_dists_buf = garment_dists_buf.unsqueeze(2).unsqueeze(0)

        garment_bc_buf = garment_bc * mask3
        garment_bc_buf[garment_bc_buf == 0] = -1
        garment_bc_buf = garment_bc_buf.unsqueeze(2).unsqueeze(0)
        ...
        fragments = Fragments(garment_ptf_buf, garment_z_buf, garment_bc_buf, garment_dists_buf)
        
        rend_img = renderer.shader(fragments, garment_mesh)
        ...
        return res_img, garment_mesh
        
    def get_mesh(self, verts, faces, bg_img, texture_rgb=None):
        verts = verts.copy()
        faces = faces.copy()
        ...
        if texture_rgb is None:
            ...
            textures = Textures(verts_rgb = torch.from_numpy(texture_rgb).float().to(self.device))
